...
describe('extractMentions', () => {
  ...
  test('bot mention detected', () => {
    const r = extractMentions([{ key: '@_bot', id: { open_id: 'ou_bot' }, name: 'Bot' }], 'ou_bot');
    expect(r.mentionedBot).toBe(true);
    expect(r.mentionList[0].isBot).toBe(true);
  });

  test('@all detection', () => {
    const r = extractMentions([{ key: '@_all', id: {}, name: '所有人' }], 'ou_bot');
...
describe('resolveMentions', () => {
  test('replaces placeholder with @name', () => {
    ...
    expect(resolveMentions('hello @_user_1', { mentions, stripBotMentions: true })).toBe(
      'hello @Alice',
    );
  });
  ...
  test('strips bot mention when flag is on', () => {
    ...
    expect(resolveMentions('@_bot help me', { mentions, stripBotMentions: true })).toBe('help me');
  });
  ...
  test('does not strip bot mention when flag is off', () => {
    ...
    expect(resolveMentions('@_bot help me', { mentions, stripBotMentions: false })).toBe(
      '@Bot help me',
    );
  });
  ...
  test('handles multiple mentions in one string', () => {
    ...
    expect(resolveMentions('@_user_1 and @_user_2', { mentions, stripBotMentions: true })).toBe(
      '@A and @B',
    );
  });

  test('empty mentions map returns unchanged', () => {
    expect(resolveMentions('hello', { mentions: new Map(), stripBotMentions: true })).toBe('hello');
  });
});
