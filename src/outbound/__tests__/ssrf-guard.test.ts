...
describe('assertPublicUrl', () => {
  ...
  test('IP literal URL pointing at private IP is rejected', async () => {
    await expect(assertPublicUrl('http://169.254.169.254/')).rejects.toThrow(/ssrf_blocked/);
  });

  test('IPv6 literal: ::1 (loopback) is rejected', async () => {
    await expect(assertPublicUrl('http://[::1]/')).rejects.toThrow(/ssrf_blocked/);
  });
  ...
  test('IPv4-mapped loopback (Node hex normalization `::ffff:7f00:1`)', async () => {
    ...
    await expect(assertPublicUrl('http://[::ffff:127.0.0.1]/')).rejects.toThrow(/ssrf_blocked/);
  });

  test('IPv4-mapped fully-expanded form', async () => {
    stubLookup([{ address: '0:0:0:0:0:ffff:7f00:1', family: 6 }]);
    await expect(assertPublicUrl('http://expanded.test/')).rejects.toThrow(/ssrf_blocked/);
  });

  test('IPv4-compatible (::127.0.0.1) delegates to v4', async () => {
    stubLookup([{ address: '::7f00:1', family: 6 }]);
    await expect(assertPublicUrl('http://v4compat.test/')).rejects.toThrow(/ssrf_blocked/);
  });

  test('NAT64 wrapping a private IPv4 (64:ff9b::10.0.0.1) is rejected', async () => {
    stubLookup([{ address: '64:ff9b::a00:1', family: 6 }]);
    await expect(assertPublicUrl('http://nat64.test/')).rejects.toThrow(/ssrf_blocked/);
  });
  ...
  test('Documentation range 2001:db8::/32 is rejected', async () => {
    ...
    await expect(assertPublicUrl('http://doc.test/')).rejects.toThrow(/ssrf_blocked/);
  });

  test('Teredo 2001::/32 is rejected', async () => {
    stubLookup([{ address: '2001:0:1234::1', family: 6 }]);
    await expect(assertPublicUrl('http://teredo.test/')).rejects.toThrow(/ssrf_blocked/);
  });
  ...
  test('link-local with Zone ID (fe80::1%eth0) is rejected', async () => {
    ...
    await expect(assertPublicUrl('http://ll.test/')).rejects.toThrow(/ssrf_blocked/);
  });

  test('ULA fc00::/7 (both fc and fd prefixes) rejected', async () => {
    stubLookup([{ address: 'fc12:3456::1', family: 6 }]);
    await expect(assertPublicUrl('http://ula-fc.test/')).rejects.toThrow(/ssrf_blocked/);

    stubLookup([{ address: 'fd00:abcd::1', family: 6 }]);
    await expect(assertPublicUrl('http://ula-fd.test/')).rejects.toThrow(/ssrf_blocked/);
  });
  ...
  test('fully-expanded ::1 (0:0:0:0:0:0:0:1) rejected via delegation', async () => {
    ...
    await expect(assertPublicUrl('http://expanded-lo.test/')).rejects.toThrow(/ssrf_blocked/);
  });

  test('garbage IPv6 string is treated as blocked (fail-closed)', async () => {
    stubLookup([{ address: 'not::an::ipv6', family: 6 }]);
    await expect(assertPublicUrl('http://garbage.test/')).rejects.toThrow(/ssrf_blocked/);
  });

  test('non-http(s) protocol is rejected', async () => {
    await expect(assertPublicUrl('file:///etc/passwd')).rejects.toThrow(/ssrf_blocked/);
    await expect(assertPublicUrl('gopher://evil.com/')).rejects.toThrow(/ssrf_blocked/);
  });

  test('hostname with no DNS records is rejected', async () => {
    stubLookup([]);
    await expect(assertPublicUrl('http://nxdomain.invalid/')).rejects.toThrow(/ssrf_blocked/);
  });
  ...
  test('CGNAT range (100.64.0.0/10) is blocked', async () => {
    ...
    await expect(assertPublicUrl('http://cgnat.test/')).rejects.toThrow(/ssrf_blocked/);
  });
