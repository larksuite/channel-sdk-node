...
function createMockClient(): {
  ...
  const imageCreate = vi.fn().mockResolvedValue({ image_key: 'img_default' });
  const fileCreate = vi.fn().mockResolvedValue({ file_key: 'file_default' });
  const httpRequest = vi.fn().mockResolvedValue(new Uint8Array([1, 2, 3]).buffer);
  const client = {
...
describe('MediaUploader.toBuffer source resolution', () => {
  ...
  test('local file path is read from fs', async () => {
    ...
    try {
      const { client, imageCreate, httpRequest } = createMockClient();
      await new MediaUploader(client, { allowedFileDirs: [os.tmpdir()] }).upload({
        kind: 'image',
        source: tmpFile,
      });
  ...
  test('non-existent local path throws upload_failed with clear message', async () => {
    ...
    await expect(
      new MediaUploader(client, { allowedFileDirs: [os.tmpdir()] }).upload({
        kind: 'image',
  ...
  describePosix('POSIX blocklist', () => {
    test.each([
      ...
      const { client } = createMockClient();
      // allowlist set (so the default-deny gate passes) — the POSIX blocklist
      // is the defense being exercised here.
      await expect(
        new MediaUploader(client, { allowedFileDirs: ['/tmp'] }).upload({
          kind: 'image',
          source: p,
        }),
      ).rejects.toMatchObject({
  ...
  describe('allowedFileDirs allowlist', () => {
    ...
    test('no allowedFileDirs configured: local path is rejected (default-deny)', async () => {
      const tmpFile = path.join(os.tmpdir(), `lfi-default-${Date.now()}.bin`);
      fs.writeFileSync(tmpFile, Buffer.from('hi'));
      try {
        const { client, imageCreate } = createMockClient();
        await expect(
          new MediaUploader(client).upload({ kind: 'image', source: tmpFile }),
        ).rejects.toMatchObject({
          code: 'upload_failed',
          message: expect.stringContaining('allowedFileDirs'),
        });
  ...
  test('URL fetch with SSRF guard enabled: pins connect target to validated IP (DNS rebinding defense)', async () => {
    ...
    // Redirects are disabled so a 3xx can't bounce the pinned-IP fetch to an
    // internal address / cloud IMDS (SSRF-via-redirect defense).
    expect(reqOpts.maxRedirects).toBe(0);
  });
